...
# Ported to Python 3 by Brandon Bremen, 2016
...
_global_symbol_table = {}


def Sym(s, symbol_table=_global_symbol_table):
...
def to_string(x):
    ...
    if x is True:
        ...
        return '"%s"' % x.encode('unicode_escape').decode().replace('"', r'\"')
    elif isa(x, list):
    ...
    while True:
        try:
            ...
            if val is not None and out:
                print(to_string(val), file=out, flush=True)
...
_append, _cons, _let = [Sym(e) for e in "append cons let".split()]
...
def let(*args):
    ...
    return [[_lambda, list(vars)] + [expand(b) for b in body]] + [expand(v) for v in vals]
